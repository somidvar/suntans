	...
	*/
	...
	if (prop->netcdfBdy) {
		...
		for (iptr = grid->celldist[1]; iptr < grid->celldist[2]; iptr++) {
			if (prop->n == prop->nstart + 1)//Added by ----Sorush Omidvar----
				printf("Type 3 boundary function is enabled with NETCDF in BoundaryScalars.\n");//Added by ----Sorush Omidvar----
		...
		for (iptr = grid->celldist[1]; iptr < grid->celldist[2]; iptr++) {
			if (prop->n == prop->nstart + 1)//Added by ----Sorush Omidvar----
				printf("Type 3 boundary function is enabled without NETCDF in BoundaryScalars.\n");//Added by ----Sorush Omidvar----
...
void BoundaryVelocities(gridT *grid, physT *phys, propT *prop, int myproc, MPI_Comm comm) {
	...
	if (prop->netcdfBdy) {
		...
		for (iptr = grid->celldist[1]; iptr < grid->celldist[2]; iptr++) {
			if (prop->n == prop->nstart + 1)//Added by ----Sorush Omidvar----
				printf("Type 3 boundary function is enabled with NETCDF in BoundaryVelocities.\n");//Added by ----Sorush Omidvar----
		...
		for (iptr = grid->celldist[1]; iptr < grid->celldist[2]; iptr++) {
			if (prop->n == prop->nstart + 1)//Added by ----Sorush Omidvar----
				printf("Type 3 boundary function is enabled without NETCDF in BoundaryVelocities.\n");//Added by ----Sorush Omidvar----
...
void WindStress(gridT *grid, physT *phys, propT *prop, metT *met, int myproc) {
	...
	if (prop->metmodel >= 2) {// Interpoalte the spatially variable wind stress onto the edges
		if (prop->n == prop->nstart + 1)//Added by ----Sorush Omidvar----
			printf("NETCDF met model is deployed in windstress boundaries.c.\n");//Added by ----Sorush Omidvar----
