...
for counter=10030:1:10099
    ...
    SapeloFlag=0;
    if SapeloFlag
        Prefix='/lustre1/omidvar/work-directory_0801/6th-New/suntans-6th-';
        CaseName='';
        CaseName=strcat(CaseName,num2str(counter));
        DataPath=strcat(Prefix,CaseName);
        DataPath=strcat(DataPath,'/InternalWaves/data/');
        CurrentPath=pwd;
        CurrentPath=strcat(CurrentPath,'\');
    else
        Prefix='F:\6th\suntans-6th-';
        CaseName='';
        CaseName=strcat(CaseName,num2str(counter));
        DataPath=strcat(Prefix,CaseName);
        DataPath=strcat(DataPath,'\InternalWaves\data\');
    ...
    if exist(strcat(DataPath,'Result_0000.nc'),'file')==0
    ...
    CaseNumber=counter;
    %WavePlotterVer12(AnalysisSpeed,FPSMovie,DiurnalTideOmega,SemiDiurnalTideOmega,WindOmega,TauMax,strcat(DataPath,'Result_0000.nc'),CurrentPath,CaseName);
    EnergyFluxCalculatorVer14(strcat(DataPath,'Result_0000.nc'),CaseNumber,...
