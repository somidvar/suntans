...
void MomentumSource(REAL **usource, gridT *grid, physT *phys, propT *prop) {
  ...
  //MR Turned off - rSponge variable not initialized correctly!
    ...
    for(jptr=grid->edgedist[0];jptr<grid->edgedist[1];jptr++) {
      ...
      ubar/=depth_face;
	  //RampFactor=prop->dt*exp(-4.0*SpongeEdgeDistance[j]/prop->sponge_distance)/prop->sponge_decay
	  
	  RampFactor=0.3*exp(-SpongeEdgeDistance[j]/prop->sponge_distance);
	  //RampFactor=0.5*(tanh((-SpongeEdgeDistance[j]+50000)/prop->sponge_distance)+1/2);
      for(k=grid->etop[j];k<grid->Nke[j];k++) 
...
void InitSponge(gridT *grid, int myproc) {
  ...
  for(j=0;j<grid->Ne;j++) {
	...
	for (CellCounter = 0; CellCounter < grid->Nc; CellCounter++)
	{
		SpongeCellDistance[CellCounter] = pow(50000-grid->xv[CellCounter], 2);
	...
	{
		if (grid->xe[EdgeCounter]<20000)
			SpongeEdgeDistance[EdgeCounter]=0;
		else
			SpongeEdgeDistance[EdgeCounter]=grid->xe[EdgeCounter]-20000;
	}
