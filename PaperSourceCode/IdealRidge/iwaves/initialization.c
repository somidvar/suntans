...
REAL ReturnDepth(REAL x, REAL y) {
	printf("It is not supposed to use the ReturnDepth\n");
	fflush(stdout);
	
	REAL Ls = 30000;
    REAL YMid=50190;
    REAL YTransition=18e3;
    REAL D0 = 3000;
    REAL Ds = 300;  
    REAL LsX = 30000;
    REAL XMid=300338;
    REAL XTransition=30e3;
    REAL XL = XMid-XTransition;
    REAL XR = XMid+XTransition;
    REAL XFlat=5e3;
	REAL DsX;
	
	if(y<=YMid)
		DsX= D0 - 0.5*(D0 - Ds)*(1 + tanh(4 * (y - YMid + YTransition) / Ls));
	else
		DsX= Ds + 0.5*(D0 - Ds)*(1 + tanh(4 * (y - (YMid+YTransition)) / Ls));
	
	REAL Bathymetry;
	
	if(x<=XL+XTransition-XFlat)
		Bathymetry=D0 - 0.5*(D0 - DsX)*(1 + tanh(4 * (x - XL) / LsX));
	else
		Bathymetry=DsX + 0.5*(D0 - DsX)*(1 + tanh(4 * (x - XR) / LsX));
	
    REAL XTemp=XL+XTransition-XFlat;
	
	if (x>XL+XTransition-XFlat && x<XR-XTransition+XFlat)
		Bathymetry=D0 - 0.5*(D0 - DsX)*(1 + tanh(4 * (XTemp - XL) / LsX));
	return Bathymetry;
	/*
	REAL Ls, xmid, Ds, D0;
...
REAL ReturnSalinity(REAL x, REAL y, REAL z) {
	...
	if (z < -D_pycnocline)
		return deltaS*pow(-z-D_pycnocline+15, alphaS);
	else
		return -0.0000001*(D_pycnocline + z) + deltaS*pow(D_pycnocline-35, alphaS);
}
