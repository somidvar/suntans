...
void MomentumSource(REAL **usource, gridT *grid, physT *phys, propT *prop) {
	...
	for (jptr = grid->edgedist[0]; jptr < grid->edgedist[1]; jptr++) {
		...
		RampFactor = 0.5*exp(-SpongeEdgeDistance[j] / prop->sponge_distance);
		for (k = grid->etop[j]; k < grid->Nke[j]; k++)
...
void InitSponge(gridT *grid, int myproc) {
	...
	REAL SpongeXCenterLocation, SpongeYCenterLocation;
	SpongeXCenterLocation = 20000;
	SpongeYCenterLocation = 0;
	if (myproc == 0)
	{
		printf("Sponge Layer Cell Location X=%f, Y=%f\t\n", 0, 0);
		printf("Sponge Layer Edge Location X=%f, Y=%f\t\n", SpongeXCenterLocation, SpongeYCenterLocation);
		fflush(stdout);
	}
	int CellCounter;
	for (CellCounter = 0; CellCounter < grid->Nc; CellCounter++)
	{
		SpongeCellDistance[CellCounter] = pow(SpongeXCenterLocation - grid->xv[CellCounter], 2);
	...
	{
		if (grid->xe[EdgeCounter] < SpongeXCenterLocation)
			SpongeEdgeDistance[EdgeCounter] = 0;
		else
			SpongeEdgeDistance[EdgeCounter] = grid->xe[EdgeCounter] - SpongeXCenterLocation;
	}
