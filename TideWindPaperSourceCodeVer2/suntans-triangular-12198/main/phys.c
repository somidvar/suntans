...
/*  for(i=0;i<Nc;i++) {
  ...
  */
  ...
  if (prop->readinitialnc){
    ...
    for(i=0;i<Nc;i++) {
      phys->h[i]=ReturnFreeSurface(grid->xv[i],grid->yv[i],grid->dv[i], prop);
...
{ 
  ...
  *prop = (propT *)SunMalloc(sizeof(propT),"ReadProperties");
  ...
  //Added by ----Sorush Omidvar----Start
  (*prop)->M2TideAmplitude =MPI_GetValue(DATAFILE,"M2TideAmplitude","ReadProperties",myproc);
  (*prop)->K1TideAmplitude =MPI_GetValue(DATAFILE,"K1TideAmplitude","ReadProperties",myproc);
  (*prop)->TidePhaseDifference =MPI_GetValue(DATAFILE,"TidePhaseDifference","ReadProperties",myproc);
  (*prop)->WindPhaseDifference =MPI_GetValue(DATAFILE,"WindPhaseDifference","ReadProperties",myproc);
  (*prop)->PycnoclineDepth =MPI_GetValue(DATAFILE,"PycnoclineDepth","ReadProperties",myproc);
  //Added by ----Sorush Omidvar----End
